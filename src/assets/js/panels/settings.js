...
class Settings {
    ...
    navBTN() {
        document.querySelector('.nav-box').addEventListener('click', e => {
            if (e.target.classList.contains('nav-settings-btn')) {
                ...
                let activeSettingsBTN = document.querySelector('.active-settings-BTN2')
                ...
                if (id == 'save') {
                    ...
                    document.querySelector('#account').classList.add('active-settings-BTN2');
                    Swal.fire({
                        position: "center",
                        icon: "success",
                ...
                if (activeSettingsBTN) activeSettingsBTN.classList.toggle('active-settings-BTN2');
                e.target.classList.add('active-settings-BTN2');
    ...
    navBTN2() {
        document.querySelector('.nav-box').addEventListener('click', e => {
            if (e.target.classList.contains('nav-settings-elfo')) {
                let id = e.target.id
        ...
        document.querySelector('.accounts-list').addEventListener('click', async e => {
            ...
            try {
                ...
                if (e.target.classList.contains("delete-profile")) {
                    popupAccount.openPopup({
                        title: 'AVISO',
                        content: 'Borrando cuenta del Launcher',
                        color: 'var(--color)'
                    })
    ...
    async javaPath() {
        ...
        document.querySelector(".java-path-set").addEventListener("click", async () => {
            ...
            } else alert("El nombre del archivo tiene que ser java o javaw");
        });
    ...
    async launcher() {
        ...
        maxDownloadFilesReset.addEventListener("click", async () => {
            let configClient = await this.db.readData('configClient')
            maxDownloadFilesInput.value = 30
            configClient.launcher_config.download_multi = 30;
            await this.db.updateData('configClient', configClient);
        })
